...
function probe(basePath: string): string | undefined {
    ...
    // These sources came from a `tsc`-built standalone repo, so sibling imports
    // name the *emitted* file: `from './fileType.js'`. Map that back onto the
    // .ts that actually exists. (run/naser import extensionless and never hit
    // this branch.)
    if (basePath.endsWith(".js")) {
        candidates.splice(1, 0, `${basePath.slice(0, -3)}.ts`);
    }
