...
suite('IDBPDatabase', () => {
  ...
  test('getAllKeysFromIndex', async (t) => {
    if (!('getAllKeys' in IDBObjectStore.prototype)) {
      t.skip();
      return;
    }
  ...
  test('add - avoid unhandled rejection', async () => {
    ...
    // Browser original used self.addEventListener('unhandledrejection').
    // Node surfaces the same condition as a process event.
    ...
    // Wait a macrotask so pending tasks are processed. Node decides a rejection
    // is unhandled once the microtask queue drains, so this must not be an await
    // on a resolved promise. (Browser original waited on requestAnimationFrame.)
  ...
  test('mode', async () => {
    db = (await openDB<TestDBSchema>(dbName, getNextVersion(), {
      upgrade(db, oldVersion, newVersion, tx) {
        ...
        // createObjectStore returns TxStores as ArrayLike<StoreNames<DBTypes>>,
        // not [Name] -- an upgrade transaction spans every store, so tx.store is
        // undefined. createIndex propagates that TxStores through unchanged.
        // Upstream asserted ['object-store'] here and only got away with it
        // because test/tsconfig.json project-references ../src, resolving it to
        // the emitted build/*.d.ts instead of the source.
        typeAssert<
          IsExact<
            typeof index,
            IDBPIndex<
...
suite('IDBPCursor', () => {
  ...
  test('continuePrimaryKey', async (t) => {
