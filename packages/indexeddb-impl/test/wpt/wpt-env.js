...
import "../../src/auto.ts";
import FakeEvent from "../../src/lib/FakeEvent.ts";
import {
    AbortError,
    ConstraintError,
    DataCloneError,
...
// Both Node and Bun ship a native File, but the structured-clone tests need
// the original bits back (see the __clone below, which reads _bits), so we
// still substitute our own.
global.File = class File extends Blob {
    constructor(bits, name, options = {}) {
        super(bits);
        this._bits = bits;
        // defineProperty, not assignment: Bun's Blob.prototype declares `name`
        // and `lastModified` as getter-only accessors, and assigning through an
        // inherited accessor with no setter throws in a class body (which is
        // always strict). Node's Blob has neither, so plain assignment worked
        // there and this difference is invisible until you run it under Bun.
